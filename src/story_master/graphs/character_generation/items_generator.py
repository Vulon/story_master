...
class MerchantStockGenerator:
    ...
    3. Read the list of all available items in the world.
    4. Pick item names that fit the merchant's description.
        Pick at least 3-4 items.
    5. Output the names of the items in XML format.
    ...
    def generate(self, merchant_description: str, location: Location) -> list[Item]:
        ...
        location_description = f"<Location>{location.name}. {location.description}. {location.interior}</Location>"
        available_items = (
            equipment_names + weapon_names + armor_names + instrument_names
        )
        available_items_string = (
            "<AvailableItems>" + " ; ".join(available_items) + "</AvailableItems>"
        )
        ...
        for raw_item_name in generated_item_names:
            match = get_close_matches(raw_item_name, available_items)[0]
